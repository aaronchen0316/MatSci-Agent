...
`gpt-5.5` is always preflighted through the configured proxy before live work. Authentication, network, proxy, and model errors block the run. `GITHUB_TOKEN` is used only for GitHub API calls and needs Contents plus Pull requests read/write permission; SSH remains the push transport.
