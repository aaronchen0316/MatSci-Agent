...
class SearchSpaceExpansionAgent:
    ...
    def expand(self, payload: SearchSpaceExpansionInput) -> SearchSpaceExpansionOutput:
        ...
        for _attempt in range(1, _MAX_EXPANSION_ATTEMPTS + 1):
            try:
                raw = self._call_llm(payload, previous_error=previous_error)
                ...
                last_error = exc
                previous_error = f"{exc.code}: {exc.message}"
                if exc.code not in {
                    "search_space_expansion_missing_targets",
                    "search_space_expansion_empty",
                }:
                    break
        ...
        raise SearchSpaceExpansionError(
            ...
            "Search-space expansion failed without a captured error.",
        )
    ...
    def _call_llm(self, payload: SearchSpaceExpansionInput, previous_error: str | None = None) -> dict[str, Any]:
        ...
        if self.provider == "openrouter":
            return self._call_openrouter(prompt_payload=prompt_payload)
        raise SearchSpaceExpansionError(
            "search_space_expansion_request_failed",
            f"Unsupported provider '{self.provider}'.",
        )

    def _call_openrouter(self, prompt_payload: dict[str, Any]) -> dict[str, Any]:
    ...
    @staticmethod
    def _prompt_payload(payload: SearchSpaceExpansionInput, previous_error: str | None = None) -> dict[str, Any]:
        ...
        if previous_error:
            ...
            prompt_payload["retry_instruction"] = (
                "Previous response failed validation. Return a non-empty formula_targets list "
                "that obeys the same constraints."
            )
    ...
    def _system_prompt() -> str:
        return (
            ...
            "chemsys is optional and advisory; formula is the source of truth.\n"
            "Return exactly: "
