...
Both commands preflight `gpt-5.5` through the configured proxy and need MP plus LLM credentials. They require a clean `multi-agent` checkout synchronized with `origin/main` and write evidence under the ignored artifact root.

All repairs start from current `origin/main`; no local `main` checkout is modified. Forward-port product changes from `multi-agent` first, then merge `origin/main` back into `multi-agent` after each repair merge. Explicit `--adopt` branches require matching stored Debugger evidence, receive fresh proof, and may get one same-branch retry. `validate-repair` publishes only fully proven `fix/<issue>` results with exact-SHA `Product CI / test` evidence. SSH pushes `fix/<issue>`, GitHub Actions validates the exact PR SHA, then the harness requests a squash merge to `main`. `GITHUB_TOKEN` is only used for GitHub API calls; no token is printed or stored in artifacts.
